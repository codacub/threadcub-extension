...
function createDownloadFromData(conversationData) {
  try {
    ...
    setTimeout(() => downloadMarkdown(tagsData), 200);
...
function generateMarkdownContent(conversationData) {
  const { title, platform, url, exportDate, messages } = conversationData;
  ...
  // Footer
  md += `\n*Export generated by ThreadCub Extension*\n`;

  return md;
}

function downloadMarkdown(conversationData) {
  try {
    const markdownContent = generateMarkdownContent(conversationData);
    const filename = generateSmartFilename(conversationData).replace('.json', '.md');
